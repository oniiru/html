...
define('TRUETHEMES_PLUGIN_DIR', TEMPLATEPATH . '/functions/truethemes/contact-form');
define('TRUETHEMES_PLUGIN_URL', get_template_directory_uri() . '/functions/truethemes/contact-form');
...
function contact_form_shortcode( $atts, $content ) {
	...
	if($captcha_public){
	    $r .= ag_recaptcha_get_html($captcha_public);
    	$r .= "<br/>";
    }	
	...
	$resp = ag_recaptcha_check_answer ($captcha_private,$_SERVER["REMOTE_ADDR"],$_POST["recaptcha_challenge_field"],$_POST["recaptcha_response_field"]); //verify captcha entered with server.
     
	     //if invalid we return error                           
 	    if (!$resp->is_valid) {
	        global $contact_form_errors;
			$contact_form_errors = new WP_Error();
			$contact_form_errors->add( $field_id, sprintf( __('Invalid CAPTCHA'), $field['label'] ) );
			$r = "<div id='contact-form-$id'>\n";
