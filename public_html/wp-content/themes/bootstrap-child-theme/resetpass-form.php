...
<div class="login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="resetpasswordform" id="resetpasswordform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'resetpass' ); ?>" method="post">
		...
		<p class="description indicator-hint"><?php _e( 'Your password should be at least 7 characters long.' ); ?></p>
<?php
do_action( 'resetpassword_form' ); // Wordpress hook
do_action_ref_array( 'tml_resetpassword_form', array( $template ) ); // TML hook
?>
		<p class="submit">
			<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" class="btn btn-custom" value="<?php esc_attr_e( 'Reset Password', 'theme-my-login' ); ?>" />
