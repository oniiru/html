<?php
	...
	function pmpro_getLevelCost(&$level, $tags = true)
	{
		...
		//initial payment
		$r = sprintf(_x('', 'Initial payment in cost text generation.', 'pmpro'), $pmpro_currency_symbol . number_format($level->initial_payment, 2));
		...
		if($level->billing_amount != '0.00')
		{
			...
			{			
				...
				{
					$r .= sprintf(__('%s/%s', 'Recurring payment in cost text generation. E.g. $5 every month for 2 more payments.', 'pmpro'), $pmpro_currency_symbol . $level->billing_amount, pmpro_translate_billing_period($level->cycle_period), $level->billing_limit, pmpro_translate_billing_period($level->cycle_period, $level->billing_limit));					
				}				
				else
				{ 
					$r .= sprintf(__('%s/%s', 'Recurring payment in cost text generation. E.g., $5 every 2 months for 2 more payments.', 'pmpro'), $pmpro_currency_symbol . $level->billing_amount, $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number), $level->billing_limit, pmpro_translate_billing_period($level->cycle_period, $level->billing_limit));					
				}
			}
			elseif($level->billing_limit == 1)
			{
				$r .= sprintf(__('%s/%s', 'Recurring payment in cost text generation. E.g. $5 after 2 months.', 'pmpro'), $pmpro_currency_symbol . $level->billing_amount, $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number));									
			}
			else
			{
				if($level->cycle_number == '1')
				{
					$r .= sprintf(__('%s/%s', 'Recurring payment in cost text generation. E.g. $5 every month.', 'pmpro'), $pmpro_currency_symbol . $level->billing_amount, pmpro_translate_billing_period($level->cycle_period));					
				}				
				else
				{ 
					$r .= sprintf(__('%s/%s', 'Recurring payment in cost text generation. E.g., $5 every 2 months.', 'pmpro'), $pmpro_currency_symbol . $level->billing_amount, $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number));					
				}			
			}
		}
		else
	...
	*/
	...
	function pmpro_checkDiscountCode($code, $level_id = NULL, $return_errors = false)
	{
		...
		{
			if($return_errors)
				return array(false, __("The code could not be found.", "pmpro"));
			else
				return false;
		}
		...
		{
			if($return_errors)
				return array(false, sprintf(__("This code goes into effect on %s.", "pmpro"), date(get_option('date_format'), $dbcode->starts)));
			else
				return false;
		}
		...
		{
			if($return_errors)
				return array(false, sprintf(__("This code expired on %s.", "pmpro"), date(get_option('date_format'), $dbcode->expires)));
			else
				return false;
		}
		...
		if($dbcode->uses > 0)
		{
			...
			{
				if($return_errors)
					return array(false, __("This code is no longer valid.", "pmpro"));
				else
					return false;
			}
		}
		...
		if(!empty($level_id) && $pmpro_check_discount_code_levels)
		{
			...
			if(empty($code_level))
			{
				if(!empty($return_errors))
					return array(false, __("This code does not apply to this membership level.", "pmpro"));
				else
					return false;
			}
		}
		...
		if(!empty($return_errors))
			return array(true, __("This code is okay.", "pmpro"));
		else
			return true;
	}
