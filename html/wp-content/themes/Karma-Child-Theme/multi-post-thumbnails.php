...
if (!class_exists('MultiThumbnails')) {
	...
	class MultiThumbnails {
		...
		//	wp_enqueue_script("featured-image-custom", $this->plugins_url('js/multi-post-thumbnails-admin.js', __FILE__), array('jquery'));
			 wp_enqueue_script('featured-image-custom', get_bloginfo('stylesheet_directory')."/js/multi-post-thumbnails-admin.js");
		}
...
function get_the_post_thumbnail_src($post_type, $thumb_id, $post_id = NULL, $size = 'post-thumbnail', $attr = '' , $link_to_original = false) {
global $id;
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			$src= wp_get_attachment_image_src( $post_thumbnail_id, 'full', false, $attr );
			return $src;
}
		/**
