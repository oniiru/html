						...
						<section class="post_content">
							...
							<form class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
								...
								<thead>
									<tr>
										<th colspan="2">
											<span class="checkoutnumber">1</span>Choose your plan
										</th>						
									</tr>
								</thead>
								<tbody>
												...
												foreach($membershipGroups as $gid=>$linkedMemberships) {
													if(in_array($pmpro_level->id, $linkedMemberships)) {
														foreach($linkedMemberships as $mid) {
															...
															<td>
																<div class="awesomeradio_container">
																	<input  class="awesomeradio" type="radio" name="level" id="level<?php echo $mid ?>" value="<?php echo $mid ?>" <?php echo (($mid==$pmpro_level->id)?' checked="checked"':'') ?>/>
																	<label  class="awesomeradio_label" for="level<?php echo $mid ?>" >
																		<p>
																			<strong><?php echo $levelDetails->name ?> -</strong>
																		</p>
								...
								<thead>
									<tr>
										<th>
											<?php echo ($pmpro_requirebilling)?'<span class="checkoutnumber">2</span>Basic Information':'<span class="checkoutnumber">2</span>Account Information'; ?>
										</th>						
									</tr>
								</thead>
								<tbody>                
								...
								<thead>
									<tr>
										<th>
											<div class="paymenttitles"><span class="checkoutnumber">3</span>Account Information</div>
										</th>						
									</tr>
								</thead>
								<tbody>
							...
							<?php echo wpautop($tospage->post_content)?>
								...
								<thead>
									<tr>
										<th><div class="paymenttitles">	<span class="checkoutnumber"><?php
...
?>
</span>Payment Information</div><span class="pmpro_thead-msg">We accept <?php echo $pmpro_accepted_credit_cards_string?>.</span></th>
									</tr>
								</thead>
								<tbody>                    
									<tr valign="top">		
...
padding-right: 50px !important;" value="<?php if($pmpro_requirebilling) { ?>Start Learning<?php } else { ?>Start Learning<?php } ?> &raquo;" />	
								...
								<?php if($pmpro_requirebilling): ?>
								<P style="text-align:center;color:white;text-shadow: 1px 1px 1px black;font-style: italic;font-family: 'Helvetica Neue' !important;">Your card won't be charged until your 7-day free trial has concluded. </p>
			<?php endif; ?>
							</form>
							...
							<?php } elseif ($pmpro_level->id == '7') {?>  <!-- start student Registration --> 
								...
								<form class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
									...
									<tbody>
										...
										<tr id="other_discount_code_tr" class="awesomediscount" style="display:block">
											<td colspan="2">
												<div class="discountfixer opaque_placeholder opaque2">
													<label for="discount_code">Registration Key * </label>
													<input class="input <?php echo pmpro_getClassForField("discount_code");?>" id="discount_code" name="discount_code" type="text" size="20" value="<?php echo esc_attr($discount_code)?>" />
										...
										<tr>
											<td>
											<?php	if(!empty($_REQUEST['classtitle']))
														$classtitle = $_REQUEST['classtitle'];
													else
														$classtitle = ""; ?>
												<div class="passfix opaque_placeholder opaque2" style="width:365px;margin-top:15px !important">
													<label for="classtitle">Course Title *</label>
													<input value="<?php echo esc_attr($classtitle)?>" id="classtitle" name="classtitle" type="text" class="input" size="30" value="" /> 
												</div>
									...
									<tbody>                
										<tr>
											<td>
												<div style="float: left; width:100%">
													<div class="opaque_placeholder" style="width:365px; margin-top:-5px !important">
														<label for="bfirstname">First Name <?php if (pmpro_getClassForField("bfirstname")=='pmpro_required') echo '&nbsp;*' ?></label>
														<input id="bfirstname" name="bfirstname" type="text" class="input" size="30" value="<?php echo esc_attr($bfirstname)?>" /> 
													</div>
													<div class="opaque_placeholder littlebillfix" style="width:365px;margin-left:20px">
									...
									<?php if(!$skip_account_fields && !$pmpro_review) { ?>
												...
												<div  class="opaque_placeholder opaque2" style="width:365px">
													<label for="bemail">E-mail Address <?php if (pmpro_getClassForField("bemail")=='pmpro_required') echo '&nbsp;*' ?></label>
													<input id="bemail" name="bemail" type="text" class="input" size="30" value="<?php echo esc_attr($bemail)?>" /> 
												</div>
												<?php
	...
	//										var level_id = jQuery('#level').val();
									...
									<div class="pmpro_submit">
										...
										<?php } else { ?>
											...
											<span id="pmpro_submit_span" <?php if(($gateway == "paypalexpress" || $gateway == "paypalstandard") && $pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
												<input type="hidden" name="submit-checkout" value="1" />		
												<input type="submit" class="submitfixer btn btn-success btn-large" style="margin-left:78px;" value="<?php if($pmpro_requirebilling) { ?>Start Learning<?php } else { ?>Start Learning<?php } ?> &raquo;" />	
				...
				<?php endif; ?>
						...
						<?php } elseif ($pmpro_level->id == '11') {?>  <!-- start student Registration --> 
							
							<h1 style="text-align: center;font-weight: 700 !important;margin-top: -15px;color: rgb(77, 77, 77);">Faculty Registration</h1>
							<h3 style="text-align:center;color:darkgrey;margin-bottom:25px"></h3>
							<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article">
									...
									<tr>
										<th colspan="2">
											<p>
												...
												Fill out the form below to access your SolidWize account.
											</p>
										</th>						
									</tr>
								</thead>
...
//										var level_id = jQuery('#level').val();
			...
			<?php endif; ?>
					...
					<footer>
						...
						<?php } elseif ($pmpro_level->id == '8') {?>  <!-- start Robotics Registration --> 
							
							<h1 style="text-align: center;font-weight: 700 !important;margin-top: -15px;color: rgb(77, 77, 77);">Best Robotics Registration</h1>
							<h3 style="text-align:center;color:darkgrey;margin-bottom:25px"></h3>
							<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article">
							...
							<form class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
								...
								<tbody>
									...
									<tr>
										<td>
										<?php	if(!empty($_REQUEST['hubname']))
													$hubname = $_REQUEST['hubname'];
												else
													$hubname = ""; ?>
											<div class="passfix opaque_placeholder opaque2" style="width:365px;margin-top:15px !important">
												<label for="hubname">Hub Name *</label>
												<input value="<?php echo esc_attr($hubname)?>" id="hubname" name="hubname" type="text" class="input" size="30" value="" /> 
											</div>
									...
									<tr>
										<td>
										<?php	if(!empty($_REQUEST['teamname']))
													$teamname = $_REQUEST['teamname'];
												else
													$teamname = ""; ?>
											<div class="passfix opaque_placeholder opaque2" style="width:365px;margin-top:15px !important">
												<label for="teamname">Team Name *</label>
												<input value="<?php echo esc_attr($teamname)?>" id="teamname" name="teamname" type="text" class="input" size="30" value="" /> 
											</div>
...
//										var level_id = jQuery('#level').val();
			...
			<?php endif; ?>
					...
					<footer>
						...
						<?php } elseif ($pmpro_level->id == '9') {?>  <!-- start student Registration --> 
							
							<h1 style="text-align: center;font-weight: 700 !important;margin-top: -15px;color: rgb(77, 77, 77);">EcoCAD Registration</h1>
							<h3 style="text-align:center;color:darkgrey;margin-bottom:25px"></h3>
							<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article">
