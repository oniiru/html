...
#icon-edit, #icon-post { background: url("<?php echo TRUETHEMES_PLUGIN_URL; ?>/images/grunion-menu-big.png") no-repeat !important; }
</style>
...
function grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_from'		=> __( 'From','truethemes' ),
		'feedback_message'		=> __( 'Message','truethemes' ),
		'feedback_date'			=> __( 'Date','truethemes' )
	);
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr( 'Resore this item from the Trash' );
				...
				echo '">' . __( 'Restore','truethemes' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently','truethemes' ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently','truethemes' ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo ( 'Mark this message as spam' );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash','truethemes' );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash','truethemes' ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam','truethemes' );
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A','truethemes' ) );
			break;
	}
}
...
function grunion_ajax_spam() {
	...
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.','truethemes' ) );

	require_once dirname( __FILE__ ) . '/truethemes-contact-form.php';
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.','truethemes') );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.','truethemes') );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.','truethemes') );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.','truethemes') );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages','truethemes' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash','truethemes' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam','truethemes' ) . ' <span class="count">';
